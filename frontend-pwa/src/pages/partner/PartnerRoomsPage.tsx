...
export default function PartnerRoomsPage() {
  ...
  return <div className="space-y-5">
    ...
    {loading?<p role="status">Đang tải...</p>:<div className="grid gap-4 md:grid-cols-2">{rooms.map(r=><section key={r.id} className="space-y-3 rounded-lg border border-border bg-surface p-4 shadow-[var(--shadow-card)]">
      <h2 className="font-bold">{r.name}</h2><p className="text-sm">{r.totalRoomCount} phòng · {r.maxOccupancy} khách/phòng · {money(r.basePrice)}/đêm{r.weekendPrice ? ` (Cuối tuần: ${money(r.weekendPrice)}/đêm)` : ''}</p><p className="text-sm text-muted">{r.status==='ACTIVE'?'Đang mở bán':'Ngừng bán'} · {r.viewDescription || 'Chưa khai báo vị trí/view'}</p>
