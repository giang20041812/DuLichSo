...
export const portalLogin = async (request: PortalLoginRequest): Promise<PortalLoginResponse> => {
  try {
    ...
    if (axios.isAxiosError(err)) {
      ...
      if (axiosError.response?.status === 404 || axiosError.code === 'ECONNABORTED' || !axiosError.response) {
        // Fallback simulation nếu endpoint chưa reload hoặc backend chưa bật
      ...
      throw {
        ...
        message: axiosError.message || 'Không thể kết nối đến máy chủ xác thực.',
    ...
    throw {
      ...
      message: 'Đã xảy ra lỗi không xác định trong quá trình xác thực.',
    } as AuthErrorResponse;
  }
};
