...
-- ============================================================================
-- TÀI KHOẢN ĐĂNG NHẬP (gộp vào để chạy 1 lần) — mật khẩu lưu dạng BCrypt.
--   ADMIN : admin@taybactrails.vn (hoặc SĐT 0988888888)   / Admin@123456
--   NCC   : SĐT của từng nhà cung cấp (xem SELECT cuối file) / Ncc@123456
-- !!! ĐỔI MẬT KHẨU sau khi đăng nhập được. Không dùng mật khẩu này cho môi trường thật.
-- ============================================================================
...
ON DUPLICATE KEY UPDATE
    password_hash = VALUES(password_hash), role = 'ADMIN', status = 'ACTIVE', provider_id = NULL;

-- Đặt mật khẩu cho mọi tài khoản NCC còn mật khẩu giữ chỗ (PLACEHOLDER) ở trên
UPDATE account
...
-- ============================================================================
-- LIÊN KẾT TIKTOK CHO CÁC ĐỊA ĐIỂM (Places Contact - TikTok Video Embed)
-- ============================================================================
...
-- ============================================================================
-- CẬP NHẬT ENUM TRẠNG THÁI BOOKING & SEED DỮ LIỆU ĐƠN ĐẶT PHÒNG (BOOKINGS)
-- ============================================================================
...
-- 1. INSERT CÁC ĐƠN ĐẶT PHÒNG VỚI MỌI TRẠNG THÁI
...
-- 2. INSERT BOOKING NIGHTS (Giá từng đêm)
...
-- 3. INSERT DỊCH VỤ ĐI KÈM (Booking Service Items)
...
-- 4. INSERT LỊCH SỬ TRẠNG THÁI (Booking Status History)
...
-- Kiểm tra: danh sách tài khoản đăng nhập
SELECT a.id AS account_id, a.phone AS login_phone, a.email AS login_email, a.role,
       a.status AS account_status, p.name AS provider_name, p.status AS provider_status
FROM account a
LEFT JOIN provider p ON p.id = a.provider_id
ORDER BY a.role, a.id;

-- Kiểm tra: số lượng video TikTok đã thêm
SELECT COUNT(*) AS total_places FROM place;
SELECT COUNT(*) AS total_tiktok_contacts FROM place_contact WHERE channel = 'TIKTOK';

-- Kiểm tra: danh sách đơn đặt phòng đã seed
SELECT 
    b.id,
    b.booking_code,
    b.status,
