...
class PartnerBookingServiceTest {
    ...
    @BeforeEach void setup() {
        ...
        lenient().when(history.getResultStream()).thenReturn(Stream.empty());
    ...
    @Test void acceptWithAlternativeRoomMovesHoldAndRepricesNights() {
        ...
        when(calendar.calendar(larger, d1, booking.getCheckOut())).thenReturn(List.of(
                new InventoryDto(d1, 2, 1, 1, 0, false, new BigDecimal("600000")),
                new InventoryDto(d2, 2, 1, 0, 1, false, new BigDecimal("700000"))));
